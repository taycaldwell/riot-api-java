...
public class GetLeagueEntriesBySummonerId extends LeagueApiMethod {
	...
	public GetLeagueEntriesBySummonerId(ApiConfig config, Platform platform, String summonerId) {
		super(config);
		setPlatform(platform);
		setReturnType(new TypeToken<Set<LeagueEntry>>() {
		}.getType());
		setUrlBase(platform.getHost() + "/lol/league/v4/entries/by-summoner/" + summonerId);
		addApiKeyParameter();
	}
}